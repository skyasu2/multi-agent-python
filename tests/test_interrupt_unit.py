...
import pytest
from graph.state import PlanCraftState, create_initial_state, PlanCraftInput
...
def test_resume_handling_form():
    ...
    state = create_initial_state("test")
    state.input_schema_name = "SomeSchema"
    state.user_input = "기존 입력"
    ...
    assert "field1: value1" in new_state.user_input
    assert new_state.input_schema_name is None
...
def test_resume_handling_option():
    ...
    assert "[선택: 선택1 - 설명]" in new_state.user_input
    assert new_state.need_more_info is False
