...
### 시장 규모 (산출 근거 필수)
- **TAM**: [전체 시장] (출처: XXX)
- **SAM**: [접근 가능 시장] (근거: TAM의 XX%인 국내 XX시장)
...
```markdown
### 기술 스택 (구체적 명시)
- **Frontend**: [React Native/Flutter/Next.js 등 구체적 기술]
- **Backend**: [FastAPI/Spring Boot/Node.js 등]
- **AI/Cloud**: [OpenAI GPT-4, AWS Lambda, ChromaDB 등 구체적 서비스명]
- **Infra/DevOps**: [Docker, Github Actions 등]
...
```markdown
### 기술적/비즈니스 리스크
| 리스크 구분 | 리스크 내용 | 영향(High/Mid/Low) | 확률(High/Mid/Low) | 대응 방안 (구체적) |
|-------------|-------------|--------------------|--------------------|--------------------|
| 기술 | [AI 환각 현상 등] | 높음 | 중간 | [RAG 기술 도입 및 검증 프로세스 추가] |
| 비즈니스 | [경쟁사 진입] | 중간 | 높음 | [초기 시장 선점 및 브랜드화] |
```
...
2. **경쟁사 표**: 4번 센션에 반드시 경쟁사 비교테이블을 포함하세요.
3. **기술 명세**: "최신 기술"이라고 뭉뚱그리지 말고 "BERT", "Cosine Similarity", "AWS Lambda" 등 엔지니어링 용어를 사용하세요.
4. **사용자 여정**: 서비스 사용 흐름을 1~5단계 User Journey로 묘사하세요.
